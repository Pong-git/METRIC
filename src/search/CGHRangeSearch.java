...
import index.CompleteGeneralHyperPlaneTree.CGHTInternalNode;
...
/**
 * CGHT（共轭梯度超平面树）的范围查询类 - 修正版
 * 使用基于截距投影的正确排除和包含规则
 */
public class CGHRangeSearch {

    /**
     * 执行 CGHT 的范围查询。
     */
    public static SearchResult rangeSearch(Object node, MetricData query, 
     ...
     * 递归搜索过程 - 修正版
     */
    private static List<MetricData> doRangeSearch(Object node, MetricData query, 
                                                 double radius, MetricDistance distance) {
        ...
        // 这里必须使用CGHTInternalNode
        CGHTInternalNode internal = (CGHTInternalNode) node;
        ...
        for (int i = 0; i < numChildren; i++) {
            ...
            //     // 检查子节点范围是否完全在查询范围内
            //     if (queryMin > nodeMin || queryMax < nodeMax) {
     ...
     * 区间无交集判断（保留，用于新的排除规则）
     */
    private static boolean isExclude(double[] queryProjection, double lower, double upper) {
        return Math.max(queryProjection[0], lower) > Math.min(queryProjection[1], upper);
    }
